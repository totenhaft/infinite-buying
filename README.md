...
1. 웹앱 접속 → **⚙️ 시작 설정 생성기** 열기
2. TQQQ/SOXL 시드와 버전(v2.2 권장) 입력 → **state.json 생성**
3. 생성된 내용을 복사 → GitHub 저장소의 `data/state.json` 열기 →
   연필(✏️) 버튼 → 전체 붙여넣기 → **Commit changes**
4. 저장소 → **Actions** 탭 → "무한매수법 데일리 가이드" → **Run workflow**
   (첫 실행 테스트)
5. 텔레그램으로 첫 가이드가 도착하면 성공! 🎉
...
1. `data/state.json` 열기 (✏️ 편집)
2. 해당 종목의 `shares`(보유수량), `avg_price`(평단),
   `total_bought`(= 평단 × 보유수량)를 증권사 앱의 실제 값으로 수정
3. `pending_orders`는 `[]`로 비우기 → Commit
4. Actions에서 **Run workflow** → 새 주문 가이드가 다시 계산됩니다

**매주 토요일 잔고 대조를 습관화하세요.**

## 5. 일시정지 / 종료

- 일시정지: `state.json`에서 해당 종목 `"active": false`
- v2.2 ↔ v3.0 전환: 사이클이 **끝난 뒤(보유 0주)** `"version"`과
  `"divisions"`(40↔20)를 함께 변경
